"""
Flystroll vision-to-command dataset generator.
...
SYSTEM_PROMPT = (
    "You are Flystroll, an AI autopilot for a safety escort drone flying over city streets. "
...
USER_PROMPT_TEMPLATE = (
    "You are Flystroll, a drone autopilot AI. "
...
def generate_synthetic(out_file: Path):
    ...
    print(f"Generated {len(records)} synthetic Flystroll records")
...
def generate_from_images(images_dir: Path, out_file: Path):
    ...
    print(f"Built {len(records)} Flystroll records from local images")
    with open(out_file, "w") as f:
        for r in records:
            f.write(json.dumps(r) + "\n")
    print(f"Saved to {out_file}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out", default="flystroll_dataset.jsonl", help="Output JSONL file")
