"""
Flystroll fine-tuning script.
Fine-tunes Pixtral 12B via the Mistral Fine-tuning API for vision-to-command output.

Usage:
  python train.py --dataset dataset/flystroll_dataset.jsonl
"""
...
def run(dataset_path: Path, epochs: int, lr: float):
    ...
    print("Creating Flystroll fine-tuning job...")
    ...
    with open(env_file, "a") as f:
        f.write(f"\nFLYSTROLL_MODEL_ID={model_id}\n")
    print(f"Model ID saved to {env_file}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--dataset", default="dataset/flystroll_dataset.jsonl")
