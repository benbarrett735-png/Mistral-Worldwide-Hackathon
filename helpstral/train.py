"""
Helpstral fine-tuning script.
Fine-tunes Pixtral 12B via the Mistral Fine-tuning API.

Usage:
  python train.py --dataset dataset/helpstral_dataset.jsonl
...
def run(dataset_path: Path, epochs: int, lr: float):
    ...
    # auto_start=True so training starts after validation (docs: default is False)
    job = client.fine_tuning.jobs.create(
        model="pixtral-12b-latest",  # vision model for image → SAFE/DISTRESS
    ...
    with open(env_file, "a") as f:
        f.write(f"\nHELPSTRAL_MODEL_ID={model_id}\n")
    print(f"Model ID saved to {env_file}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--dataset", default="dataset/helpstral_dataset.jsonl")
